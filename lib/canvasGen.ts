...
async function renderBanner(size: AdSize, brief: Brief, bgImg: HTMLImageElement | null): Promise<string> {
  ...
  ov.addColorStop(0.5, primary + "88");
  ov.addColorStop(1, primary + "ee");
  ctx.fillStyle = ov;
  ctx.fillRect(0, 0, w, h);

  const setShadow = () => { ctx.shadowColor = "rgba(0,0,0,0.9)"; ctx.shadowBlur = 4; ctx.shadowOffsetX = 1; ctx.shadowOffsetY = 1; };
  ...
  if (isWide) {
    ...
    if (appName) {
      ...
      ctx.fillText(appName, textX, h / 2);
      textX += ctx.measureText(appName).width + 20;
    }
    ctx.font = `700 ${hlSize}px Inter, Arial, sans-serif`;
    ctx.fillStyle = "white"; setShadow();
    ctx.fillText(headline.slice(0, 50), textX, h / 2);
    clrShadow();
    const ctaX = w - ctaW - pad;
    ctx.fillStyle = accent;
    drawRoundRect(ctx, ctaX, (h - ctaH) / 2, ctaW, ctaH, ctaR);
    ctx.fill();
    ctx.font = `700 ${ctaSize}px Inter, Arial, sans-serif`;
    ...
    if ((brief.app_store_url || brief.play_store_url) && h > 200 && !isTiny) {
      ...
      const drawBadge = (l1: string, l2: string) => {
        ...
        ctx.font = "400 7px Inter, Arial"; ctx.fillStyle = "#aaa"; ctx.textAlign = "center";
        ctx.fillText(l1, bx + 50, badgeY + 8);
        ctx.font = "700 9px Inter, Arial"; ctx.fillStyle = "white";
...
export async function generateAllBanners(brief: Brief, bgDataUrl: string | null): Promise<GeneratedBanner[]> {
  ...
  for (const size of AD_SIZES) {
    const dataUrl = await renderBanner(size, brief, bgImg);
